...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (CategoryState.playlist == 4)
		{
				
			if (StoryMenuState.weekUnlocked[18] || isDebug)
				addWeek(['Lo-Fight', 'Overhead', 'Ballistic'], 18, ['whitty', 'whitty', 'whitty-crazy']);
			if (StoryMenuState.weekUnlocked[19] || isDebug)
				addWeek(['Improbable-Outset', 'Madness'], 19, ['trickyMask', 'tricky']);
			if (StoryMenuState.weekUnlocked[19] || isDebug)
				addWeek(['Improbable-Outset-Duet', 'Madness-Duet'], 19, ['trickyMask', 'tricky']);
		...
		if (CategoryState.playlist == 9)
		{
			if (StoryMenuState.weekUnlocked[20] || isDebug)
				addWeek(['Carol-Roll', 'Philly-Carol', 'Blammed-Carol'], 20, ['carol']);
		}
